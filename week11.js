...
function makeZero() {
	...
	paragraph.textContent = 'Я изменила содержимое параграфа';
}
...
function makeThree() {
	...
	header.innerHTML = '<h1 id="practicum3">Заголовок изменен!</h1>';
}
...
function makeFive() {
	const input = document.querySelector('#practicum5');
...
function makeTwentyTwo() {
	...
	const attribute = img.hasAttribute('src');
	//3 шаг: Найдите элемент, в который нужно вставить значение
	const div = document.getElementById('result22');
	...
	const p = document.createElement('p');
	p.textContent = 'Атрибут "src" присутствует: ' + attribute;
	div.append(p);
}
...
function makeTwentyFive() {
	...
	p.textContent = 'Добавленный атрибут: ' + attribute;
}
...
function makeTwentySeven() {
	...
	const p = document.createElement('p');
	p.textContent = 'Содержит класс: ' + result;
	divResult.append(p);
}
...
//Подсказка: Используйте метод addEventListener для добавления обработчика события. Чтобы проверить работу слушателя, нужно напечатать один символ и более в поле ввода и убрать фокус с инпута, кликнув на любое место страницы.
const div = document.getElementById('div30'); 
